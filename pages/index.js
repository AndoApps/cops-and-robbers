...
export default function Home() {
  ...
  const handleJoin = () => {
    ...
    if (!joinCode.trim() || joinCode.trim().length < 4) { setError('Please enter a valid game code'); return }
    router.push(`/student?code=${joinCode.trim().toUpperCase()}&name=${encodeURIComponent(studentName.trim())}`)
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #1a0a00 50%, #0a0a0f 100%)' }}>
      ...
      {step === 'choose' && (
        <div className="flex flex-col gap-4 w-full max-w-xs">
          <button
            ...
            style={{ background: '#1a1a2e', border: '2px solid #ef4444', color: '#ef4444' }}
          >
            👮 I&apos;M THE TEACHER
          </button>
          <button
            onClick={goStudent}
            className="w-full py-4 rounded-xl text-lg font-bold tracking-widest transition-all duration-200 hover:scale-105"
            style={{ background: '#1a1a2e', border: '2px solid #fbbf24', color: '#fbbf24' }}
          >
            🦹 I&apos;M A STUDENT
          </button>
        </div>
          ...
          <div className="rounded-2xl p-6" style={{ background: '#111', border: '1px solid #333' }}>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-lg text-white placeholder-gray-600 outline-none text-2xl tracking-widest text-center font-bold"
                style={{ background: '#1a1a2e', border: '1px solid #374151' }}
                maxLength={6}
                onKeyDown={e => e.key === 'Enter' && handleJoin()}
              />
      ...
      <p className="mt-12 text-gray-700 text-xs tracking-widest">© ANDOAPPS</p>
    </div>
  )
}
