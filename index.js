...
export default function Student() {
  ...
  const [phase, setPhase] = useState('joining') // joining, setup, playing, paused, ended
  const [gameId, setGameId] = useState(null)
  const [studentId, setStudentId] = useState(null)
  const [grid, setGrid] = useState(null) // 49 cells
  ...
  const initGrid = () => {
    // Create shuffled set of 49 items
  ...
  const subscribeToMyRecord = (sid) => {
    supabase.channel(`my-record-${sid}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'students', filter: `id=eq.${sid}` }, (payload) => {
        ...
        // Handle notifications from teacher
        if (s.notification) {
  ...
  const handleLeaveRequest = async () => {
    setLeavePrompt(true)
    await supabase.from('students').update({ leaving: true }).eq('id', studentId)
  }
    ...
    <div className="min-h-screen flex flex-col p-3" style={{ background: '#0a0a0f', maxWidth: '500px', margin: '0 auto' }}>
          ...
          <div key={row} className="grid grid-cols-8 gap-1 mb-1">
            ...
            {COLS.map((col, colIdx) => {
              ...
              return (
                <div
                  key={ref}
                  className={`aspect-square rounded flex flex-col items-center justify-center text-xs transition-all select-none ${called ? 'called' : ''} ${isPickable ? 'available' : ''}`}
                  style={{
      ...
      {(phase === 'setup' || phase === 'waiting') && !isCuffed && (
        <div className="flex gap-3 mb-3">
          <button
            ...
            className="flex-1 py-2 rounded-xl text-sm font-bold tracking-wider"
            style={{ background: '#1a1a2e', border: '1px solid #374151', color: '#9ca3af' }}
          >
            🎲 AUTO
          </button>
          <button
            onClick={handleCuffIt}
            className="flex-1 py-3 rounded-xl font-bold tracking-wider text-lg"
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="rounded-xl p-2 text-center" style={{ background: '#111', border: '1px solid #333' }}>
          ...
          <p className={`font-bold text-sm ${scoreAnimating ? 'animate-slot' : ''}`} style={{ color: '#22c55e' }}>
            {formatDollars(pointsBanked)}
          </p>
        </div>
        <div className="rounded-xl p-2 text-center" style={{ background: '#111', border: '1px solid #333' }}>
          <p className="text-xs text-gray-600 mb-1">💵</p>
          <p className={`font-bold text-sm ${scoreAnimating ? 'animate-slot' : ''}`} style={{ color: '#fbbf24' }}>
            {formatDollars(pointsInPlay)}
          </p>
        </div>
...
function StudentEndScreen({ myPlace, myScore, podiumData, podiumVisible, studentId }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: '#0a0a0f' }}>
      {podiumVisible && (
        <>
          <h2 className="text-2xl font-bold tracking-widest mb-8 animate-fade-in" style={{ color: '#fbbf24' }}>
            🚨 GAME OVER 🚨
          </h2>
            ...
            {[1, 0, 2].map((rankIdx) => {
              ...
              return (
                <div key={rankIdx} className={`text-center animate-slide-up`} style={{ animationDelay: `${rankIdx * 0.5}s` }}>
                  ...
                  <div
                    className={`rounded-xl px-4 py-3 ${heights[rankIdx]} flex flex-col items-center justify-center w-24`}
                    style={{
                      background: isMe ? '#1a1500' : '#1a1a2e',
                      border: `2px solid ${isMe ? '#fbbf24' : '#374151'}`,
                    }}
                  >
